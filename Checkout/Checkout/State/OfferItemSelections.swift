...
class OfferItemSelections2: IdentifiableItemSelections<OfferItem> {
}
...
class OfferItemSelections: ObservableObject, CustomStringConvertible {
    ...
    var items: [OfferItem]  { Array(offerItemsMap.values) }
    
    init(items: [OfferItem] = []) {
        self.offerItemsMap = items.autoDictionary()
    }
